...
export interface HomePostPagination {
	page: number;
	pageSize: number;
	totalPages: number;
	totalPosts: number;
...
export interface HomePostsResult {
	pinned: CollectionEntry<"posts">[];
	recent: CollectionEntry<"posts">[];
	pagination: HomePostPagination;
}

/**
 * 首页专用文章分组。置顶文章只在第一页出现，最近文章始终排除全部置顶内容。
 * 通用排序接口保持不变，避免影响归档、推荐和文章前后导航。
 */
export async function getHomePosts(
	page: number,
	pageSize: number,
	pinnedLimit: number,
): Promise<HomePostsResult> {
	const safePageSize = Math.max(1, Math.trunc(pageSize));
	const safePinnedLimit = Math.max(0, Math.trunc(pinnedLimit));
	const sorted = await getSortedPosts();
	const allPinned = sorted.filter((post) => post.data.pinned);
	const allRecent = sorted.filter((post) => !post.data.pinned);
	const totalPages = Math.max(1, Math.ceil(allRecent.length / safePageSize));
	const safePage = Math.min(Math.max(1, Math.trunc(page)), totalPages);
	const start = (safePage - 1) * safePageSize;

	return {
		pinned: safePage === 1 ? allPinned.slice(0, safePinnedLimit) : [],
		recent: allRecent.slice(start, start + safePageSize),
		pagination: {
			page: safePage,
