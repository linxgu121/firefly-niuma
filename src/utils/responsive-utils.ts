...
export function generateGridClasses(config: ResponsiveSidebarConfig): {
	...
	if (
		...
		if (config.tabletSidebar === "right") {
			...
			gridCols =
				"grid-cols-1 md:grid-cols-[minmax(0,1fr)_15rem] xl:grid-cols-[15rem_minmax(0,1fr)_15rem]";
		} else {
			// 平板端显示左侧栏（默认）: 769-1279px [左侧栏+内容], 1280px+ [左+中+右]
			gridCols =
				"grid-cols-1 md:grid-cols-[15rem_minmax(0,1fr)] xl:grid-cols-[15rem_minmax(0,1fr)_15rem]";
		}
	} else if (config.hasLeftComponents && !config.hasRightComponents) {
		// 仅左侧边栏: 769px+显示左+中，768-以下单列
		gridCols = "grid-cols-1 md:grid-cols-[15rem_minmax(0,1fr)]";
	} else if (!config.hasLeftComponents && config.hasRightComponents) {
		// 仅右侧边栏: 769px+显示中+右，768-以下单列
		gridCols = "grid-cols-1 md:grid-cols-[minmax(0,1fr)_15rem]";
	}
