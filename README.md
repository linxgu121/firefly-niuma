...
- **发文日历**：重写为细长极简版（单月视图、圆点标记、多日期筛选、按发布时间升序显示最多 5 篇）
...
- **平衡动效**：桌面保留打字机、切页、卡片光泽、轻量樱花与交互式壁纸水波，移动端和减少动态模式自动降级
...
`ambientEffects.rippleRefraction` 控制交互式壁纸水波。CPU 使用两个低分辨率 `Float32Array`
高度场分别计算慢速涟漪和快速鼠标尾迹，WebGL 只读取合成高度纹理并对壁纸做折射采样。
指针移动产生轻微波动，点击产生较强波纹并在 400ms 后触发一次弱回弹。
樱花越过配置的水面线时会消失并触发更小的扩散波纹，落点由 Worker 回传，不需要在主线程重复计算粒子位置。

默认每 5 个 CSS 像素对应一个模拟格点，仅在桌面 `overlay` 和 `fullscreen` 模式启用；
波场平静后会停止逐帧更新。移动端、减少动态模式、隐藏标签页会回到静态壁纸，WebGL 不可用时使用 Canvas 2D 高光线作为回退。
